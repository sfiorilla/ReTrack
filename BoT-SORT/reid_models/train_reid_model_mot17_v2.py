...
parser.add_argument('--restopt',  type=str, required=False, help='If continue training')
...
MODELNAME=args.model if args.model else "mot17_full_model.pth.tar"
RESTOPT=args.restopt.strip().lower() == 'true'
RESUME=args.restore
...
class MOT17ReID(ImageDataset):
    ...
    def __init__(self, root='', **kwargs):
        ...
        if not osp.isdir(self.data_dir):
            warnings.warn(
                'The current data structure is deprecated. Please put data folders in "bounding_box_train","bounding_box_test" and "query_set" '
            )
        ...
        super(MOT17ReID, self).__init__(train, query, gallery, **kwargs)

    def process_dir(self, dir_path, relabel=False):
        img_paths = glob.glob(osp.join(dir_path, '*.jpg'))
        #pattern = re.compile(r'([-\d]+)_(\d)')
        pattern = re.compile(r'([1-9]\d*)_MOT')

        pid_container = set()
        for img_path in img_paths:
            pid = list(map(str, pattern.search(img_path).groups()))[0]
            #if pid == -1:
        ...
        for img_path in img_paths:
            ...
            camid=1
            if "query_set" in dir_path:
                camid=2
            
            data.append((img_path,pid,camid)) #added a fake camid

        return data


torchreid.data.register_image_dataset('MOT17_ReID', MOT17ReID)
...
class CenterLoss(nn.Module):
    ...
    def __init__(self, num_classes=751, feat_dim=2048, use_gpu=True):
        ...
        self.num_classes = num_classes
        print(f"Center Loss num_classes {self.num_classes}")
...
def custom_feature_extraction(self, data_loader):
    f_, pids_, camids_ = [], [], []
...
#engine.Engine._evaluate._feature_extraction = custom_feature_extraction
...
class CostumEngine(engine.Engine):
    ...
    def __init__(
        ...
        self.criterion_r = ReconstructionLoss(self.use_gpu) if self.weight_r >0 else None
    ...
    def parse_data_for_eval(self, data):
        ...
        pids = data['pid']
        camids = data['camid']
        return imgs, np.array(pids), camids
...
for param in model.encoder.base_model.bottleneck.parameters():
    param.requires_grad = True

for param in model.encoder.base_model.classifier.parameters():
    param.requires_grad = True

#Unfreeze all norm layers in the model
...
#if you are not using reconstruction loss, freeze decoder layer.
#if wr<=0:
...
#    factor=0.1, patience=5, threshold=0.000001,
#    threshold_mode='rel', cooldown=0, min_lr=0.000000035
#)
...
def load_model_state(model, opt, scheduler, filepath):
    ...
    #skip last layer.
    filtered_dict = {k: v for k, v in checkpoints["model_state_dict"].items() if "module.encoder.base_model.classifier" not in k}
...
engine = CostumEngine(
    ...
    load_optimizer2= RESUME=='TRUE' and RESTOPT=='TRUE'
)

print("Test with weights from Market1501")
engine.run(test_only=True,rerank=False,dist_metric=DIST)

print('\nValues inside (parentesis) are AVG values')
num_trainable_layers = len([p for p in model.parameters() if p.requires_grad])
print(f'There are {num_trainable_layers} trainable layers.')



####UPDATE BN Layers before 
def update_bn_momentum(model, new_momentum, reset=False):
...
update_bn_momentum(model, new_momentum=0.5, reset=False) #Keep market1501 data as well.
model.train()
with torch.no_grad():
    for batches in datamanager.train_loader:
        inputs = batches["img"]
        _,_ = model(inputs)

#compute metrics only with the first 

print("Test with New BN layers")
engine.run(test_only=True,rerank=False,dist_metric=DIST)

#Restore normal training settings
update_bn_momentum(model, new_momentum=0.1)
for param in model.parameters():
    param.requires_grad = True 
