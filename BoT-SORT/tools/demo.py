...
def image_demo(predictor, vis_folder, current_time, args):
    ...
    if args.save_result:
        ...
        # NEW
        fprefixes = args.path.split("/")[-2].split("-")
        fname = fprefixes[0]+"-"+fprefixes[1]
        suffixes = ["DPM","FRCNN","SDP"]
        os.chdir("..") # go up to parent
        dest_output_path = os.getcwd()+"/TrackEval/data/trackers/mot_challenge/MOT17-train/last_execution/data"
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            # Detect objects
            outputs, img_info = predictor.inference(frame, timer, frame_id)
            scale = min(exp.test_size[0] / float(img_info['height'], ), exp.test_size[1] / float(img_info['width']))
