...
def image_track(predictor, vis_folder, args):
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            ...
            trackerTimer.tic()
            online_targets = tracker.update(detections, img_info["raw_img"], logger, frame_id )
            trackerTimer.toc()
    ...
    os.chdir("..") # go up to parent
    dest_output_path = os.getcwd()+"/TrackEval/data/trackers/mot_challenge/MOT17-test/last_execution/data"
    os.makedirs(dest_output_path, exist_ok=True)
    fprefixes=args.name.split("-")
    fname = fprefixes[0]+"-"+fprefixes[1]
    
    for suffix in ["DPM","FRCNN","SDP"]:
        res_file = osp.join(dest_output_path, f"{fname}-{suffix}.txt")
        with open(res_file, 'w') as f:
            f.writelines(results)
    ...
    #assert False, "stop"
    # NEW
    #prefixes = args.path.split("/")[-2].split("-")
    #fname = fprefixes[0]+"-"+fprefixes[1]
    #suffixes = ["DPM","FRCNN","SDP"]
...
if __name__ == "__main__":
    ...
    mainTimer = Timer()
    mainTimer.tic()
    print(F"MOT{MOT} SEQUENCES to EVALUATE: {seqs}")
    for ext in seqs_ext:
        for i in seqs:
            if i < 10:
        ...
        break #exit after the first iteration of for ext
