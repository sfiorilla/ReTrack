...
class STrack(BaseTrack):
    ...
    def update_features(self, feat):
        ...
        # exponential moving average update with adaptive weights.
        if self.smooth_feat is None:
            self.smooth_feat = feat
        else:
            self.smooth_feat = self.alpha * self.smooth_feat + (1 - self.alpha) * feat
            # cos_distance = cdist(self.smooth_feat, feat, 'cosine')
            # a, b = 0.05, 0.295  # Min and Max of new range.
            # x_min, x_max = np.max(cos_distance),np.min(cos_distance)
            # scaled_value = a + ((cos_distance - x_min) * (b - a) / (x_max - x_min))
            # gamma = self.alpha + np.clip(scaled_value, 0, b )
            # self.smooth_feat = gamma * self.smooth_feat + (1 - gamma) * feat
